...
const Events = () => {
  ...
  const [events, setEvents] = useState([
    ...
    { id: 2, title: "QRBASE STRATEGY MEET", date: "05 Feb 2026", requested: "JAN 30, 10:15 AM" },
  ]);
  ...
  const handleApply = () => {
    ...
    const dateStr = `${selectedDate.getDate()} ${months[selectedDate.getMonth()].charAt(0) + months[selectedDate.getMonth()].slice(1).toLowerCase().substring(0, 2)} ${selectedDate.getFullYear()}`;
    const newEvent = {
      id: Date.now(),
      ...
      <header className="flex justify-between items-center px-12 py-5 bg-white shadow-sm sticky top-0 z-50 shrink-0">
        ...
        <nav className="hidden md:flex items-center gap-10">
          <div className="flex gap-8 text-sm font-bold items-center uppercase tracking-widest">
            <button onClick={() => navigate('/dashboard')} className="text-blue-600 hover:text-gray-400 transition-colors bg-transparent border-none p-0 cursor-pointer font-bold uppercase tracking-widest outline-none">Dashboard</button>
            <button onClick={() => navigate('/events')} className="text-blue-600 hover:text-gray-400 transition-colors bg-transparent border-none p-0 cursor-pointer font-bold uppercase tracking-widest outline-none">Events</button>
            <button onClick={() => navigate('/role')} className="text-blue-600 hover:text-gray-400 transition-colors bg-transparent border-none p-0 cursor-pointer font-bold uppercase tracking-widest outline-none">Team</button>
            <button onClick={() => navigate('/attendance')} className="text-blue-600 hover:text-gray-400 transition-colors bg-transparent border-none p-0 cursor-pointer font-bold uppercase tracking-widest outline-none">Attendance</button>
          </div>
        ...
        <aside className="group absolute inset-y-0 left-0 z-20 flex flex-col justify-between w-20 hover:w-64 bg-[#1e293b] transition-all duration-300 m-4 rounded-[2rem] shadow-2xl overflow-hidden">
          <div className="flex flex-col gap-2 p-4 mt-2">
            {sidebarLinks.map((link) => (
              <button 
                ...
                className={`flex items-center w-full py-3.5 px-3 rounded-2xl transition-all active:scale-95 ${
          ...
          <div className="p-4 mb-2">
            <button 
              ...
              className="flex items-center w-full py-3.5 px-3 rounded-2xl text-red-400 hover:bg-red-500 hover:text-white transition-all active:scale-95"
            >
        ...
        <main className="flex-1 ml-24 p-8 overflow-y-auto">
          <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            ...
            <section className="flex flex-col gap-4">
              ...
              <div className="bg-white rounded-[2.5rem] shadow-xl p-8 h-[580px] flex flex-col justify-between">
                <div>
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <input 
                      ...
                      className="w-full bg-[#f1f5f9] border-2 border-slate-100 rounded-xl p-4 text-xs outline-none focus:border-[#2563eb] focus:bg-white font-bold transition-all" 
                ...
                <button 
                  onClick={handleApply} 
                  className="w-full bg-[#1e293b] text-white py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-[#2563eb] shadow-lg transition-all active:scale-95"
                >
                  + Add Event
                </button>
              </div>
            ...
            <section className="flex flex-col gap-4">
              ...
              <div className="bg-white rounded-[2.5rem] shadow-xl p-8 h-[580px] flex flex-col gap-6">
                ...
                <div className="flex gap-2">
                  <select 
                    className="flex-1 bg-[#f1f5f9] border-none rounded-xl p-3 text-[10px] font-black text-[#1e40af] outline-none cursor-pointer appearance-none"
                  ...
                  <select 
                    className="w-24 bg-[#f1f5f9] border-none rounded-xl p-3 text-[10px] font-black text-[#1e40af] outline-none cursor-pointer appearance-none"
              ...
              <div className="bg-white rounded-[2.5rem] shadow-xl p-8 h-[580px] flex flex-col">
                ...
                <div className="relative mb-6">
                  ...
                  <input 
                    ...
                    className="w-full bg-[#f1f5f9] border-2 border-slate-50 rounded-2xl py-3 pl-11 pr-4 text-[10px] font-black uppercase tracking-widest outline-none focus:border-[#2563eb] focus:bg-white transition-all shadow-sm"
                  />
                </div>
                ...
                <div className="flex-1 overflow-y-auto pr-2 space-y-4 scrollbar-thin scrollbar-thumb-slate-200">
                  {filteredEvents.length > 0 ? (
                    filteredEvents.map((event) => (
                      <div key={event.id} className="p-5 border-2 border-slate-50 rounded-[2rem] bg-white flex justify-between items-center group hover:border-blue-100 transition-all active:scale-[0.98]">
                        <div className="flex flex-col gap-2">
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div className="flex flex-col">
                              ...
                              <span className="text-[7px] font-medium text-slate-300 uppercase">Req: {event.requested}</span>
                            </div>
                          </div>
                        </div>
                        <button onClick={() => setEvents(events.filter(e => e.id !== event.id))} className="bg-[#1e293b] text-white p-3 rounded-xl hover:bg-red-500 transition-colors shadow-sm active:scale-90">
                    ...
                    <div className="h-full flex flex-col items-center justify-center text-slate-300 gap-4 opacity-60">
                      <Icon path="M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607z" className="w-16 h-16 stroke-[1.5]" />
                      <span className="text-[10px] font-black uppercase tracking-widest text-center px-4">No events match your search</span>
                    </div>
                  )}
                </div>
              </div>
