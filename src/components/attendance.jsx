          ...
          <div className="bg-white w-full max-w-md rounded-[2.5rem] shadow-2xl p-8 animate-in fade-in zoom-in duration-200">
            ...
            <form onSubmit={handleCreateEntry} className="space-y-4">
              <div>
                ...
                <input required value={newEntry.name} onChange={e => setNewEntry({...newEntry, name: e.target.value})} className="w-full bg-[#f1f5f9] rounded-xl p-3 text-xs font-bold outline-none border-2 border-transparent focus:border-blue-500 transition-all" placeholder="E.G. JOHN DOE" />
              </div>
              <div>
              ...
              <div>
                ...
                <input required value={newEntry.dateRange} onChange={e => setNewEntry({...newEntry, dateRange: e.target.value})} className="w-full bg-[#f1f5f9] rounded-xl p-3 text-xs font-bold outline-none border-2 border-transparent focus:border-blue-500 transition-all" placeholder="E.G. 01 FEB - 05 FEB" />
              </div>
        ...
        <nav className="hidden md:flex items-center gap-10">
          <div className="flex gap-8 text-sm font-bold items-center uppercase tracking-widest">
            {sidebarLinks.map(link => (
              <button key={link.name} onClick={() => navigate(link.path)} className="text-blue-600 hover:text-gray-400 transition-colors bg-transparent border-none p-0 cursor-pointer font-bold uppercase tracking-widest outline-none">{link.name}</button>
            ))}
          </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {filteredRequests.length === 0 ? (
                    <div className="col-span-full flex flex-col items-center justify-center text-slate-300 py-20">
                      <Icon path="M9.172 16.172a4 4 0 015.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" className="w-16 h-16 mb-4 opacity-40" />
                      ...
                      <div key={req.id} className="bg-[#f8fafc] border-2 border-transparent hover:border-blue-100 p-6 rounded-[2.5rem] shadow-sm transition-all group relative animate-in fade-in slide-in-from-bottom-2 duration-300">
                        ...
                        {activeTab === 'Pending' && (
                          <div className="flex gap-3 mt-4">
                            <button onClick={() => handleStatusUpdate(req.id, 'Confirmed')} className="flex-1 bg-blue-600 text-white py-3 rounded-2xl text-[9px] font-black uppercase tracking-widest shadow-lg shadow-blue-100 hover:bg-blue-700 transition-all active:scale-95">Approve</button>
                            <button onClick={() => handleStatusUpdate(req.id, 'Rejected')} className="flex-1 bg-white border-2 border-slate-100 text-red-500 py-3 rounded-2xl text-[9px] font-black uppercase tracking-widest hover:bg-red-50 transition-all active:scale-95">Reject</button>
                          </div>
                        )}
                      </div>
                    ))
